-- Drop the insecure view
DROP VIEW IF EXISTS public.global_leaderboard;

-- Create a secure function to get all user leaderboard data
CREATE OR REPLACE FUNCTION public.get_global_leaderboard()
RETURNS TABLE (
    user_id uuid,
    total_points integer,
    meditation_streak integer,
    last_meditation_date date,
    fitness_streak integer,
    last_fitness_date date,
    active_streak integer,
    total_sessions bigint,
    total_meditation_time bigint,
    total_fitness_sessions bigint,
    total_fitness_time bigint,
    display_name text
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, auth
AS $$
BEGIN
    RETURN QUERY
    SELECT 
        au.id as user_id,
        COALESCE(up.total_points, 0) as total_points,
        COALESCE(up.meditation_streak, 0) as meditation_streak,
        up.last_meditation_date,
        COALESCE(up.fitness_streak, 0) as fitness_streak,
        up.last_fitness_date,
        COALESCE(GREATEST(up.meditation_streak, up.fitness_streak), 0) as active_streak,
        COALESCE(ms.total_sessions, 0) as total_sessions,
        COALESCE(ms.total_meditation_time, 0) as total_meditation_time,
        COALESCE(fs.total_fitness_sessions, 0) as total_fitness_sessions,
        COALESCE(fs.total_fitness_time, 0) as total_fitness_time,
        public.safe_username_from_auth(au.id) as display_name
    FROM auth.users au
    LEFT JOIN public.user_points up ON au.id = up.user_id
    LEFT JOIN (
        SELECT 
            ms_inner.user_id,
            COUNT(*) as total_sessions,
            SUM(ms_inner.duration) as total_meditation_time
        FROM public.meditation_sessions ms_inner
        WHERE ms_inner.status = 'completed'
        GROUP BY ms_inner.user_id
    ) ms ON au.id = ms.user_id
    LEFT JOIN (
        SELECT 
            fs_inner.user_id,
            COUNT(*) as total_fitness_sessions,
            SUM(fs_inner.duration) as total_fitness_time
        FROM public.fitness_sessions fs_inner
        GROUP BY fs_inner.user_id
    ) fs ON au.id = fs.user_id
    ORDER BY COALESCE(up.total_points, 0) DESC, au.created_at DESC;
END;
$$;

-- Create a secure view that uses the function
CREATE VIEW public.global_leaderboard AS
SELECT * FROM public.get_global_leaderboard();