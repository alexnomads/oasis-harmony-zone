...
-- Update total_points to be calculated from both meditation and fitness
-- First, let's create a function to update fitness points
...
SELECT 
    ...
    -- User info
    au.email,
    COALESCE(up_profiles.nickname, SPLIT_PART(au.email, '@', 1)) as display_name
FROM user_points up
...
LEFT JOIN user_profiles up_profiles ON up.user_id = up_profiles.user_id
